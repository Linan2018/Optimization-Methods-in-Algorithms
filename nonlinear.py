...
if __name__ == '__main__':
    ...
    f_diff = lambda x: np.asarray([[-x[1]], [-x[0]]])
    g = lambda x : x[0] ** 2 + x[1] ** 2 - 1
    g_diff = lambda x: np.asarray([[2*x[0]], [2*x[1]]])
    history["ip1"] = ip(f, g, start_default, 3, 2, 1, max_iterations=max_iterations)
    history["ip2"] = ip(f, g, -1*start_default, 3, 2, 1, max_iterations=max_iterations)

    plot_result([history['ip1'], history['ip2']], f, filename="ip")
    history["cp"] = cutting_plane(f, f_diff, g, g_diff)
    plot_result_v2(history["cp"], f, filename="cp")